...
const StackShipsBattle = () => {
  ...
  const buyShot = () => {
    ...
    if (currentScore >= 10) {
      ...
      Alert.alert(
        'Not enough score',
        `You need 10 points in your ${
          useGameScore ? 'game' : 'total'
        } score to buy a shot.`,
      );
    }
  };
  ...
  const convertScoreToShots = () => {
    ...
    if (shotsToAdd > 0) {
      ...
      Alert.alert(
        'Shots Added',
        `Converted ${shotsToAdd * 5} points to ${shotsToAdd} shots.`,
      );
    } else {
      Alert.alert(
        'Not Enough Score',
        'You need at least 5 points to convert to a shot.',
      );
    }
  };
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      enemyShips.forEach(ship => {
        if (
          ship.isAlive &&
          Math.abs(ship.position.x._value - playerShipPosition.x._value) <
            SHIP_SIZE &&
          Math.abs(ship.position.y._value - playerShipPosition.y._value) <
            SHIP_SIZE
        ) {
          handleGameCompletion();
          clearInterval(interval);
      ...
      <ImageBackground
        ...
        {isMenuOpen && (
          <View style={styles.menuContainer}>
            ...
            <TouchableOpacity
              style={[styles.menuItem, styles.exitMenuItem]}
              onPress={exitGame}>
...
const styles = StyleSheet.create({
  ...
  playerShip: {
    ...
    position: 'absolute',
    // bottom: '10%', // Adjust this value to move ship up/down
